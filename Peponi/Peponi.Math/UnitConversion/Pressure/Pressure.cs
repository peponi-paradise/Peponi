...
internal class Pressure
{
    ...
    {
        var multiplierFrom = convertFrom switch
        {
            PressureUnit.at => at,
            ...
            _ => throw new ArgumentException($"{convertFrom} is not supported")
        };

        var multiplierTo = convertTo switch
        {
            PressureUnit.at => at,
            PressureUnit.atm => atm,
            PressureUnit.Pa => 1,
            ...
            PressureUnit.inAq60F => inAq60F,
            PressureUnit.ftAq60F => ftAq60F,
            _ => throw new ArgumentException($"{convertTo} is not supported")
        };
