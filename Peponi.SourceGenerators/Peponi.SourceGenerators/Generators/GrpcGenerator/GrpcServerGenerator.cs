...
public sealed partial class GrpcServerGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var targetInfos = syntaxProvider.Where(static item => item.Info is not null);
        CodeBuilder codeBuilder = new CodeBuilder();
        SourceProductionContext sourceContext;
        BeginExecute(codeBuilder);
        context.RegisterSourceOutput(targetInfos, (productionContext, targetInfo) =>
        {
            Execute(targetInfo.Info, codeBuilder);
            sourceContext = productionContext;
        });
        EndExecute(sourceContext, codeBuilder);
    }
    ...
    private static (GrpcServerInfo Info, Diagnostic Error) GetGrpcServerTarget(GeneratorSyntaxContext context)
    {
        ...
        else
        {
            ...
            {
                foreach (var methodInfo in typeSymbol.BaseType.GetType().DeclaringType.GetMethods())
                {
                    if (methodInfo.Name == "BindService")
                    {
                        baseName = typeSymbol.BaseType!.GetType().DeclaringType.Name;
                    }
                }
            }
            if (string.IsNullOrEmpty(baseName))
            {
                // return (null, DiagnosticMapper.Create(typeSymbol, CouldNotFindProperTarget));
                return (null, default)!;
            }

            GrpcServerInfo protobufTarget = new(serverMode, typeSymbol.ContainingNamespace.ToDisplayString(), typeSymbol.Name, baseName);
